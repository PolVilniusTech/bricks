...
<head>
  ...
  <title>Bricks File Upload #4</title>
		...
		<p>
			<?php
				...
				if(isset($_POST['upload'])) {
					...
					$file_meta = finfo_open(FILEINFO_MIME_TYPE);
					$file_mime = finfo_file($file_meta, $_FILES['userfile']['tmp_name']);
					# try {
					# $size = getimagesize($_FILES['userfile']['tmp_name']);
					# } catch (Exception $e) { 
					# echo 'Exception for sure: ' . $e->getMessage() . "\n";
					# }
					if (in_array($file_mime, $valid_mime_type)) {
